...
{
   [
      System.Runtime.InteropServices.Guid("e81a67e4-7792-45b6-8c09-2522524cd85f"),
      Rhino.Commands.CommandStyle(Rhino.Commands.Style.ScriptRunner)
   ]
   public class AddLayoutCommand : Command
   {
      static AddLayoutCommand _instance;
      public AddLayoutCommand()
      {
         _instance = this;
      }

      ///<summary>The only instance of the MyCommand1 command.</summary>
      public static AddLayoutCommand Instance
      {
      ...
      public void drawBlock(RhinoDoc doc, string labelName, string area, int panelNum, PanelParameters panelParas)
      {
         ...
         if(text.Length > 50)
         {
                plane.Origin = new Point3d(158, 26 + 1.3, 0);
                id = doc.Objects.AddText(text, plane, 1.3, font, false, false);
            }
         else
         {
         ...
         id = doc.Objects.AddText(text, plane, height, font, false, false);
            ...
            text = "APPROVED BY";
            plane.Origin = new Point3d(138, 61 + height, 0);
            id = doc.Objects.AddText(text, plane, height, font, false, false);

            text = "NAME";
            plane.Origin = new Point3d(138, 56 + height, 0);
            id = doc.Objects.AddText(text, plane, height, font, false, false);

            text = "SIGNATURE";
            plane.Origin = new Point3d(138, 51 + height, 0);
            id = doc.Objects.AddText(text, plane, height, font, false, false);

            text = "DATE";
            plane.Origin = new Point3d(138, 41 + height, 0);
            id = doc.Objects.AddText(text, plane, height, font, false, false);
